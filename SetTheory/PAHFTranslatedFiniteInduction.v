...
Proof.
  ...
  assert (hzero : BProv Ax_s [generation] (subst substZero below)).
  {
    unfold below.
    rewrite substZero_hfStrongBelowAt.
    set (lowLtZero := ltTermAt (tVar 0) tZero).
    assert (hbody : BProv Ax_s (map (rename S) [generation])
      (pImp lowLtZero psi)).
    {
      set (D := lowLtZero :: map (rename S) [generation]).
      ...
      unfold D in hpsi.
      exact (BProv_impI Ax_s (map (rename S) [generation])
        lowLtZero psi hpsi).
    }
    exact (BProv_allI_of_sentences Ax_s [generation]
      (pImp lowLtZero psi) sentence_ax_s hbody).
  }
  assert (hsuccBody : BProv Ax_s
    (below :: map (rename S) [generation])
    (subst substSuccVar below)).
  {
    set (Sctx := below :: map (rename S) [generation]).
    ...
    assert (hgenerationS : BProv Ax_s Sctx (rename S generation)).
    {
      unfold Sctx.
      apply BProv_ass.
      simpl; auto.
    ...
    {
      apply (hcurrent Sctx).
      - unfold generation in hgenerationS. exact hgenerationS.
  ...
  assert (hsuccImp : BProv Ax_s (map (rename S) [generation])
    (pImp below (subst substSuccVar below))).
  {
    exact (BProv_impI Ax_s (map (rename S) [generation])
      below (subst substSuccVar below) hsuccBody).
  }
  assert (hsucc : BProv Ax_s [generation]
    (pAll (pImp below (subst substSuccVar below)))).
  {
    exact (BProv_allI_of_sentences Ax_s [generation]
      (pImp below (subst substSuccVar below)) sentence_ax_s hsuccImp).
  }
  assert (hallBelow : BProv Ax_s [generation] (pAll below)).
  { exact (BProv_Ax_s_induction_rule [generation] below hzero hsucc). }
  assert (hpsiBody : BProv Ax_s (map (rename S) [generation]) psi).
  {
    set (Q := map (rename S) [generation]).
    assert (hgenerationQ : BProv Ax_s Q (rename S generation)).
    {
      unfold Q.
      apply BProv_ass.
      simpl; auto.
