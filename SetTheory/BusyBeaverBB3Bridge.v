...
From CoqBB3 Require Import BB3_Statement BB3_Encodings TM BB3_Theorem.

Module BB := BusyBeaver.
Module BBKV := BusyBeaverKnownValues.

Module BusyBeaverBB3Bridge.
...
Lemma natOfSt_stOfNat_lt_three : forall q,
  (q < 3)%nat -> natOfSt (stOfNat q) = q.
Proof.
...
Definition tmOfMachine (M : BB.machine 3) : TM Σ :=
  fun s i => transOfAction (BB.transition 3 M (natOfSt s) (boolOfSigma i)).
...
Lemma tapeOfConfig_step :
  forall (M : BB.machine 3) (cfg : BB.config 3) q q',
    BB.cfg_state _ cfg = Some q ->
    BB.action_next
      (BB.transition 3 M q (BB.Tape.read (BB.cfg_tape _ cfg) (BB.cfg_head _ cfg))) =
      Some q' ->
    mov Σ
...
Proof.
  ...
  rewrite hState.
  set (bit := BB.Tape.read (BB.cfg_tape 3 cfg) (BB.cfg_head 3 cfg)).
  change (BB.Tape.read (BB.cfg_tape 3 cfg) (BB.cfg_head 3 cfg)) with bit in hNext |- *.
  rewrite hNext.
  set (a := BB.transition 3 M q bit) in *.
  ...
  - destruct (Z.eqb (rel + -1) 0) eqn:hEq.
    + apply Z.eqb_eq in hEq.
      replace (BB.cfg_head 3 cfg - 1 + rel)%Z with
        (BB.cfg_head 3 cfg) by (unfold BB.move_apply; lia).
      rewrite BB.Tape.read_write_same.
      reflexivity.
    + apply Z.eqb_neq in hEq.
      rewrite BB.Tape.read_write_of_ne.
      * replace (BB.cfg_head 3 cfg + (rel + -1))%Z with
          (BB.cfg_head 3 cfg - 1 + rel)%Z by lia.
        reflexivity.
      * lia.
  - destruct (Z.eqb (rel + 1) 0) eqn:hEq.
    + apply Z.eqb_eq in hEq.
...
Proof.
  ...
  rewrite (natOfSt_stOfNat_lt_three q hqLt).
  rewrite hNext.
  f_equal.
  f_equal.
...
Proof.
  ...
  - cbn in hState.
    ...
    destruct (BB.cfg_state 3 prev) as [old|] eqn:hOld.
    + unfold BB.Machine.step in hState.
      rewrite hOld in hState.
      set (bit := BB.Tape.read (BB.cfg_tape 3 prev) (BB.cfg_head 3 prev)) in *.
      destruct (BB.action_next (BB.transition 3 M old bit)) as [next|] eqn:hNext.
      * inversion hState; subst q.
        econstructor.
        -- apply ih. reflexivity.
        -- apply step_some_bridge.
           ++ exact (BBKV.run_state_lt 3 M n old hOld).
           ++ exact hOld.
           ++ exact hNext.
      * discriminate hState.
    + rewrite (BB.Machine.step_of_halted 3 M prev hOld) in hState.
      rewrite hOld in hState. discriminate hState.
Qed.

Lemma step_none_bridge :
  forall (M : BB.machine 3) (cfg : BB.config 3) q,
    (q < 3)%nat ->
    BB.cfg_state _ cfg = Some q ->
    BB.action_next
      (BB.transition 3 M q (BB.Tape.read (BB.cfg_tape _ cfg) (BB.cfg_head _ cfg))) =
...
Lemma local_halt_event_to_coqbb3 :
  forall (M : BB.machine 3) n q,
    BB.cfg_state _ (BB.Machine.run M n) = Some q ->
...
Proof.
  ...
  - cbn in hNextNone.
    ...
    set (bit := BB.Tape.read (BB.cfg_tape 3 (BB.Machine.run M n))
      (BB.cfg_head 3 (BB.Machine.run M n))) in *.
    destruct (BB.action_next (BB.transition 3 M q bit)) as [next|] eqn:hActionNext.
    + discriminate hNextNone.
    + apply step_none_bridge.
      * exact (BBKV.run_state_lt 3 M n q hSome).
      * exact hSome.
      * exact hActionNext.
Qed.

Lemma local_halt_event_time_le_twenty_one :
  forall (M : BB.machine 3) n q,
    BB.cfg_state _ (BB.Machine.run M n) = Some q ->
    BB.cfg_state _ (BB.Machine.run M (S n)) = None ->
    (S n <= 21)%nat.
Proof.
  ...
  pose proof (local_halt_event_to_coqbb3 M n q hSome hNone) as hHalt.
  pose proof (BB3_upperbound (tmOfMachine M) n hHalt) as hBound.
  unfold BB3_minus_one in hBound. lia.
Qed.

Lemma local_halted_has_early_event :
  forall (M : BB.machine 3) t,
    BB.cfg_state _ (BB.Machine.run M t) = None ->
    exists n q,
      (n < t)%nat /\
      BB.cfg_state _ (BB.Machine.run M n) = Some q /\
      BB.cfg_state _ (BB.Machine.run M (S n)) = None.
Proof. exact (BB.Machine.halted_has_early_event 2). Qed.
...
Lemma local_run_eq_early_halt_event :
  forall (M : BB.machine 3) t n q,
    (n < t)%nat ->
    BB.cfg_state _ (BB.Machine.run M n) = Some q ->
    BB.cfg_state _ (BB.Machine.run M (S n)) = None ->
    BB.Machine.run M t = BB.Machine.run M (S n).
Proof. exact (fun M t n _ hlt _ hEvent =>
  BB.Machine.run_eq_early_halt_event 3 M t n hlt hEvent). Qed.

Theorem three_state_halting_time_bound_event :
  forall (M : BB.machine 3) t,
    BB.cfg_state _ (BB.Machine.run M t) = None ->
    exists h,
      (h <= 21)%nat /\
...
Proof.
  ...
  exists (S n).
  repeat split.
  - exact (local_halt_event_time_le_twenty_one M n q hSome hEvent).
  - exact hEvent.
  - apply local_run_eq_early_halt_event with (q := q); assumption.
Qed.

Theorem upperBound_three : forall score,
  BB.AttainableScore 3 score -> (score <= 6)%nat.
Proof.
  intros score [M [t [hState hScore]]].
  destruct (three_state_halting_time_bound_event M t hState) as
    [h [hLe [hHalt hRun]]].
  rewrite hRun in hScore.
  pose proof (BBKV.three_state_halted_score_le_six_by_time M h hLe hHalt) as hScoreLe.
  lia.
Qed.
