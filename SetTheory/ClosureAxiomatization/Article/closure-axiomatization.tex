...
assistant, organized as a small reusable library: a shallow embedding proving the
...
$\ZF \proves \varphi \Rightarrow \T \models \varphi$; a from-scratch proof of
...
and finally the \emph{deep reverse}: the finite recursion theorem rendered with
genuine first-order formulas and carried out inside an arbitrary first-order
$\ZF$-model, which yields the converse
$\T \proves \varphi \Rightarrow \ZF \proves \varphi$ and hence the full
machine-checked deductive equivalence
$\T \proves \varphi \Leftrightarrow \ZF \proves \varphi$.
...
theories, the forward syntactic statement
$\ZF \proves \varphi \Rightarrow \T \proves \varphi$, and --- via the recursion
theorem built as genuine first-order formulas inside an arbitrary $\ZF$-model ---
the converse, closing the machine-checked equivalence
$\T \proves \varphi \Leftrightarrow \ZF \proves \varphi$. We will walk through
each layer, quote the actual definitions and theorem statements, and explain the
design decisions and how the one genuine obstacle --- the deep reverse --- was
eventually overcome.
...
(\S\ref{sec:mech-calculus}), and completeness, compactness, and the forward
syntactic direction (\S\ref{sec:mech-compl}). Section~\ref{sec:mech-deeprev} is
the deep reverse: the recursion theorem as a first-order formula, the converse
syntactic direction, and the full equivalence. Section~\ref{sec:remaining} states
precisely what is proven. Section~\ref{sec:conclusion} reflects on both
punchlines.
...
The formalization is organized by topic in three repository subtrees:
\texttt{Logic/FirstOrder/Coq/}, \texttt{SetTheory/ZF/Coq/}, and
\texttt{SetTheory/ClosureAxiomatization/Coq/}.  Together they contain seven
Rocq/Coq source files organized as a small library (see the appendix for the
module map). This section covers the two self-contained files that prove the
semantic equivalence directly: \texttt{Forward.v} and \texttt{Reverse.v}.
...
Coq predicates rather than over first-order \emph{formulas}. The library removes
that caveat by building a genuine deep embedding of the first-order language of
set theory (\texttt{Fol.v} --- syntax, renaming, free variables, sealing, a
formula enumeration, and Tarski satisfaction over an arbitrary structure) and
re-proving the trade from schemas that quantify over syntactic formulas
(the \cq{DeepForward} section of \texttt{Equivalence.v}).
...
\texttt{Equivalence.v} then re-derives Pairing, Union, first-order Replacement,
and Infinity from \emph{these} schemas. The new obligation, compared to the shallow
...
\paragraph{Why the reverse direction is harder to deepen.} The forward trade ports
to the deep setting cheaply because every relation it uses is a short first-order
formula. The reverse direction does \emph{not}: its essential use of Replacement
collects the family $\{W_n\}$ via the map $\underline n \mapsto W_n$, and that map
is first-order definable only through the \emph{recursion theorem}.
\texttt{Reverse.v} deliberately sidesteps the recursion theorem by iterating on
the meta-level \cq{nat}; rendering the same construction first-order means
formalizing the recursion theorem's defining formula --- a heavy object-level
construction, and the genuine first-order content of the reverse direction.
Section~\ref{sec:mech-deeprev} carries it out.

% =====================================================================
\section{Mechanization III: a proof calculus, soundness, and a cross-theory bridge}\label{sec:mech-calculus}
% =====================================================================

So far everything has been semantic (about models). \texttt{Calculus.v} builds
the \emph{syntactic} layer: an actual proof calculus over \cq{form}, its
admissible rules, and a soundness theorem; \texttt{Equivalence.v} then draws a
first bridge between the two theories at the level of provability.
...
i.e.\ the recursion theorem as discussed; it is proved in
\S\ref{sec:mech-deeprev}.

% =====================================================================
\section{Mechanization IV: G\"odel completeness, compactness, and the syntactic equivalence}\label{sec:mech-compl}
% =====================================================================

\texttt{Completeness.v} proves the converse of soundness ---
\textbf{G\"odel completeness} --- from scratch, then leverages it to obtain
compactness for sentence theories; the forward \emph{syntactic} equivalence
$\ZF \proves \varphi \Rightarrow \T \proves \varphi$ follows in
\texttt{Equivalence.v}. (\texttt{Completeness.v} is fully generic: it applies to
any theory over this language, and nothing in it mentions set theory.)
...
The construction leans on the usual admissible rules, all provided by
\texttt{Calculus.v}: weakening, proof by contradiction and double-negation
elimination, a notion of
consistency \cq{Con G := ~ Prov G fBot}, the equality kit
(symmetry/transitivity/congruence --- available
...
coincides with its membership in the theory \cq{T}. It is proved by structural
induction on the formula, with the substitution \cq{s} generalized: a quantifier
case recurses on the \emph{body} of the quantified formula, at a substitution
extended by the witness variable (\cq{rename_inst_up} rewrites the instantiated
renaming into that form):
...
extension of any consistent set. The construction is carried out once, in the
generality that the endgame needs: relative to a base theory $B$ of
\emph{sentences} --- axioms with no free variables, as $\ZF$ and $\T$ will be ---
together with a finite working context. Provability and consistency relative to
$B$ mean: some finite batch of $B$-axioms joins the derivation.
...
The construction needs an enumeration of all formulas; \texttt{Fol.v} builds one
from Cantor's pairing function and proves it surjective:
...
Then a chain \cq{chainB B L0 n} processes formulas one at a time: at each step it
decides the next formula (adding it or its negation, by the Lindenbaum step
\cq{BCon_cons_or}) and, when it adds an existential, also adds a
\emph{fresh-witness} instance (the eigenvariable
and Henkin-witness lemmas, from \cq{Prov_rename} plus a freshness analysis of free
variables). Freshness is exactly where the sentence restriction earns its keep:
the witness variable must be fresh for everything in play, and for an infinite
base theory that mentions all variables there would be none --- but a witness
fresh for the \emph{finite} list of formulas added so far is automatically fresh
with respect to the sentence base $B$, since sentences contribute no free
variables to clash with. The limit theory \cq{Tinf} is shown to satisfy all five
hypotheses of \cq{model_exists} --- consistent, complete, deductively closed, and
Henkin for both $\exists$ and $\forall$. Combining:
...
\subsection{Completeness, finite and infinite}

Finite-context completeness is now the \emph{empty-base} instance $B = \emptyset$:
\cq{BProv (fun _ => False) G phi} is plain \cq{Prov G phi} (\cq{BProv_empty}), so
\cq{model_of_BCon} specializes to \cq{model_of_con} (every consistent finite
context has a model), and contraposing gives \cq{completeness}, hence
\cq{prov_iff_valid}. At the other end, taking $L_0$ to carry a negated goal
sentence gives the compactness-style lift to genuinely infinite theories --- such
as $\ZF$ and $\T$, whose Separation/Closure schemas have infinitely many
instances:
...
lemmas, then reuses the derived satisfaction facts of the forward trade (\cq{sat_Pair},
...
half of the statement that $\ZF$ and $\T$ are the same theory --- the half that
goes through cleanly, because \cq{Tmodel_sat_ZF} could reuse the deep forward
derivations wholesale. The other half needs the dual model inclusion, and
that is where the recursion theorem finally has to be paid for.

% =====================================================================
\section{Mechanization V: the deep reverse --- the recursion theorem as a first-order formula}\label{sec:mech-deeprev}
% =====================================================================

\texttt{Zf.v} and \texttt{Equivalence.v} close the converse. By
\cq{theory_equiv} (or directly by \cq{completeness_inf} plus soundness), the
syntactic statement $\T \proves \varphi \Rightarrow \ZF \proves \varphi$ follows
from the dual of \cq{Tmodel_sat_ZF}: \emph{every first-order $\ZF$-model
satisfies every instance of \cq{Closure_form}}. That is the \emph{deep reverse}
direction, and it is genuinely hard, for a reason that has been visible since
\S\ref{sec:mech-deep}.

\subsection{The obstacle, and the shape of the solution}
...
collected by an external recursion. A first-order $\ZF$-model is not handed such
a meta-recursion --- a nonstandard model has stages at nonstandard levels that no
meta-level \cq{nat} ever reaches --- so it must \emph{build} the family
internally, as a set-coded function. Doing that is precisely the content of the
\textbf{finite recursion theorem} on $\omega$:

\begin{itemize}[leftmargin=1.4em]
\item encode functions as sets of Kuratowski pairs;
\item formalize the ``$m$-approximation'' of the iteration and prove existence
  and uniqueness of approximations by $\omega$-induction, with every
  set-existence step passing through the model's \emph{first-order}
  Separation and Replacement --- i.e.\ through genuine formulas;
\item feed the (now provably functional) stage relation to first-order
  Replacement over $\omega$ and take the union.
\end{itemize}

One design decision deserves emphasis. We do \emph{not} write raw object
derivations in the calculus \cq{Prov}; we prove \emph{satisfaction in an
arbitrary first-order model}, quantifying over models inside Coq, and let the
completeness theorem of \S\ref{sec:mech-compl} convert model-theoretic truth
into provability. This is the standard model-theoretic route, and it is what the
completeness layer was built for --- but nothing conceptual is skipped by it:
the recursion theorem's \emph{defining formula} still has to be constructed, as
an actual De~Bruijn \cq{form}, because the model's Separation and Replacement
schemas accept nothing else. The heavy lifting is exactly where the textbook
says it is; only the final conversion from truth to proof is outsourced to
G\"odel.

\subsection{An internal $\omega$ and a definable-induction schema}

Fix a model $(V,\in)$ of the first-order $\ZF$ axioms (the file extracts them
from \cq{ZFax_s} through \cq{bridge_*} lemmas; notably the list is
$\{$Ext, Sep, Pair, Union, Inf, Repl$\}$ --- \emph{neither Powerset nor
Regularity is needed}, so the deep reverse inherits the sharpening of
\S\ref{sec:powerset-loadbearing} at the first-order level). Inside $V$ the file
rebuilds the basic algebra --- pairs, unions, successors \cq{vsucc}, and
Kuratowski pairs \cq{kpair} with machine-checked injectivity \cq{kpair_inj} ---
and then carves the internal $\omega$ out of the Infinity witness \emph{by a
formula}: $\omega$ is the set of members of the inductive witness that belong to
every inductive set, where ``inductive'' is itself a \cq{form} (\cq{fInd}). The
payoff is the \textbf{definable-induction schema}, the engine that replaces the
meta-level \cq{nat}:
...
Induction holds for \emph{any property expressible by a formula with
parameters} --- and only for those, which is exactly the discipline the
first-order setting imposes. The internal arithmetic needed later
(transitivity of $\omega$ and of each natural, irreflexivity
\cq{nat_no_self}, successor injectivity \cq{succ_inj_nat}) is proved through
\cq{omega_ind} with small explicit formulas, Foundation-free, mirroring the
numeral lemmas of \texttt{Reverse.v} one level down.

\subsection{The recursion theorem's defining formula}

The construction is parameterized by the closure relation: a formula \cq{psiC}
with argument slots $0,1$ and parameters \cq{eC} (the instance of the schema
being proved). Two preliminary canonicalizations tame the choice functions that
\texttt{Reverse.v} used freely:

\begin{itemize}[leftmargin=1.4em]
\item the \emph{canonical predecessor set}
  $\mathrm{predSet}(v) = \{z : z \prec v\}$ (\cq{predSet})
  --- carved by Separation inside an $\varepsilon$-chosen bound from
  set-likeness, but unique by Extensionality, so the $\varepsilon$ leaks nothing
  into the object level; its graph ``$y = \mathrm{predSet}(x)$'' is a formula
  (\cq{psiPS}), provably functional, so first-order Replacement can push it
  through a set;
\item the canonical one-step operator
  $\mathrm{gstep}(t) = t \cup \bigcup\{\mathrm{predSet}(v) : v \in t\}$, with
  ``$y = \mathrm{gstep}(t)$'' again a formula (\cq{fStepF}).
\end{itemize}

A library of formula macros with satisfaction specs (\cq{fEmptyF}, \cq{fSingF},
\cq{fUPairF}, \cq{fKPairF}, \cq{fPairMemF}, \cq{fSuccF}, \dots) then assembles
the \textbf{approximation predicate}: $\mathrm{Approx}(f,m)$ (\cq{Approx}) says
$f$ is (the graph of) a function on $\{0,\dots,m\}$ recording the stages
$s,\,g(s),\,g(g(s)),\dots$ --- five clauses: functionality, domain bound, the
base pair $\langle 0,s\rangle \in f$, domain coverage, and the recurrence
$f(k{+}1) = g(f(k))$. Each clause is both a Coq predicate and a \cq{form}
(\cq{fAppC1}--\cq{fAppC5}, packaged as \cq{fApproxF}), with a spec lemma
equating the two readings. This \cq{fApproxF} --- roughly the formula
$\exists f\,[\mathrm{Fun}(f) \wedge \mathrm{dom}\,f = m{+}1 \wedge f(0)=s
\wedge \forall k{<}m\ f(k{+}1)=g(f(k))]$ of the textbook proof --- \emph{is}
the recursion theorem's defining formula, built for real.

Existence and agreement of approximations are proved by \cq{omega_ind}
(existence extends an $m$-approximation by one new Kuratowski pair, where
injectivity \cq{kpair_inj} and the arithmetic lemmas guarantee the extension
stays functional; agreement runs a second internal induction with $f, f'$ as
parameters). Agreement is the crucial one: it makes the \textbf{stage relation}
\[
  \Theta(y,m) \;:\equiv\; m \in \omega \wedge \exists f\,
  (\mathrm{Approx}(f,m) \wedge \langle m,y\rangle \in f)
\]
\emph{functional} (\cq{theta_functional}) --- exactly the premise first-order
Replacement demands. Replacement applied to $\Theta$ (as the formula
\cq{fThetaF}) over $\omega$ collects the stages; the union of the image is the
closure set:
...
\subsection{The converse, and the equivalence}

Bridging back through \cq{Closure_form} gives \cq{ZFmodel_sat_Closure} (every
$\ZF$-model satisfies every sealed Closure instance), hence
\cq{ZFmodel_sat_T} --- every $\ZF$-model is a $\T$-model, the dual of
\cq{Tmodel_sat_ZF}. Soundness plus \cq{completeness_inf} then deliver the
converse syntactic direction, and with \cq{ZF_implies_T} the headline:
...
The two theories also provably have \emph{the same models}
(\cq{T_ZF_same_models}), so \cq{T_iff_ZF} falls out a second time as an
instance of the abstract \cq{theory_equiv} --- a pleasant cross-check that the
general machinery and the concrete construction agree
(\cq{T_iff_ZF_via_theory_equiv}).

% =====================================================================
\section{What is proven}\label{sec:remaining}
% =====================================================================
...
forward trade from \emph{first-order} schemas & machine-checked & \texttt{Equivalence.v} \\
sound ND calculus; $\ZF\proves\varphi \Rightarrow \T\models\varphi$ & machine-checked & \texttt{Calculus.v}, \texttt{Equivalence.v} \\
...
$\ZF \proves \varphi \Rightarrow \T \proves \varphi$ & machine-checked & \texttt{Equivalence.v} \\
deep reverse: every FO $\ZF$-model satisfies Closure & machine-checked & \texttt{Zf.v} \\
$\T \proves \varphi \Rightarrow \ZF \proves \varphi$ & machine-checked & \texttt{Equivalence.v} \\
$\T \proves \varphi \Leftrightarrow \ZF \proves \varphi$ & machine-checked & \texttt{Equivalence.v} \\
\bottomrule
\end{tabular}
\end{center}

The table is closed everywhere: both semantic directions, both syntactic
directions, and the logical infrastructure (soundness, completeness,
compactness, deductive equivalence of same-model theories) that connects them.
...
There is no \cq{Admitted} and no custom \cq{Axiom} anywhere in the development.
...
that make the quotient an honest type.

A word on \cq{constructive_indefinite_description} (with \cq{epsilon}), since it is
easy to misread. It is \emph{metatheoretic} choice --- Hilbert's $\varepsilon$ in the
ambient logic --- and lives at a different level from the set-theoretic Axiom of
Choice, which (as \S\ref{sec:theories} explained) appears in neither $\T$ nor $\ZF$
here, and which these certificates accordingly do not list. Moreover the two are not
even the same \emph{kind} of principle. Hilbert's $\varepsilon$ is a uniform,
definable selector: it returns a canonical witness for every existence statement at
once. In the set-theoretic reading that is the \emph{global} form of choice ---
$\varepsilon x.(x \in a)$ is a global choice function, picking from every nonempty
set --- rather than the set-by-set Axiom of Choice; the two are inter-definable over
a base set theory, and global choice is stronger in character than AC (though
conservative over $\ZFC$). So the metatheory is, if anything, helping itself to
\emph{more} choice than the object theories ever mention --- but only ever in the
harmless \emph{descriptive} way: $\varepsilon$ is used solely to \emph{name}
witnesses already proved to exist (turning the existential axioms into the operators
\cq{power}, \cq{sep}, \cq{host}, \dots) and to pick canonical representatives for the
quotient term model. It never proves a choice statement that was not already present,
and it adds no object-level set-theoretic strength. Beyond these standard classical
axioms the development assumes no set-theoretic universe or large-cardinal strength.
...
deductively equivalent --- and finally to the deep reverse: the finite recursion
theorem built as an actual first-order formula (Kuratowski pairs, an internal
$\omega$ with a definable-induction schema, the approximation predicate and its
functionality) and run inside an arbitrary first-order $\ZF$-model, so that
G\"odel completeness converts model-theoretic truth into the converse
$\T \proves \varphi \Rightarrow \ZF \proves \varphi$. The result is the full
machine-checked deductive equivalence
$\T \proves \varphi \Leftrightarrow \ZF \proves \varphi$, with no admitted goals
and only the standard classical axioms --- a complete and honest account of what
it takes to certify that a slogan (``one closure schema'') really is $\ZF$ in
disguise.
...
The development is seven Rocq/Coq files organized by topic across
\texttt{Logic/FirstOrder/Coq/}, \texttt{SetTheory/ZF/Coq/}, and
\texttt{SetTheory/ClosureAxiomatization/Coq/}: three files of generic
first-order logic (usable for any theory over the language of one binary
relation and equality), one file of first-order $\ZF$, one file for the
axiomatization $\T$ and the equivalence, and the two original self-contained
shallow files. The import DAG:
\[
  \texttt{Fol} \to \texttt{Calculus} \to \texttt{Completeness}, \qquad
  \{\texttt{Fol}, \texttt{Calculus}\} \to \texttt{Zf}, \qquad
  \text{all} \to \texttt{Equivalence}.
\]
...
\texttt{Fol.v} & \emph{generic}: deep embedding of first-order logic over $\{\in,=\}$ (\cq{form}, \cq{rename}, \cq{Free}, \cq{seal}, the formula enumeration \cq{Enum}, Tarski \cq{Sat}, \cq{Sat_ext}/\cq{Sat_rename}, \cq{relOf}). \\
\texttt{Calculus.v} & \emph{generic}: classical ND calculus \cq{Prov}, admissible rules (weakening, cut, deduction, \cq{Prov_rename}, the equality kit, Henkin cores), \cq{soundness}. \\
\texttt{Completeness.v} & \emph{generic}: from-scratch G\"odel \cq{completeness}/\cq{prov_iff_valid}; \cq{completeness_inf} (compactness for sentence theories); \cq{theory_equiv}. \\
\texttt{Zf.v} & first-order $\ZF$: the axioms as formulas, \cq{ZFax_s}, extraction bridges; internal model mathematics --- set algebra with \cq{kpair_inj}, internal $\omega$ and the definable-induction schema \cq{omega_ind}, the formula-macro library, \cq{Approx}/\cq{fApproxF} with existence and agreement, \cq{theta_functional}, \cq{ClosureFO_of_ZF}. \\
\texttt{Equivalence.v} & the axiomatization $\T$: deep forward trade; \cq{ZF_provable_holds_in_T}; \cq{Closure_form}; \cq{Tax_s}; \cq{Tmodel_sat_ZF}/\cq{ZFmodel_sat_T}; \cq{ZF_implies_T}; \cq{T_implies_ZF}; \cq{T_iff_ZF}. \\
\bottomrule
\end{tabular}
\end{center}

\texttt{Forward.v} and \texttt{Reverse.v} are independent (no inter-file
\cq{Require}) and need only the standard library; the library files import along
the DAG above. \texttt{Completeness.v} additionally uses the standard
classical/extensionality modules for the quotient term model.
