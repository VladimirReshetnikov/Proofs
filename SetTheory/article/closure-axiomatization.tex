...
assistant, organized in five developments: a shallow embedding proving the
...
goes through cleanly, because \cq{Tmodel_sat_ZF} could reuse \texttt{Deep.v}'s
forward derivations wholesale. The other half needs the dual model inclusion, and
...
The fifth file, \texttt{DeepReverse.v}, closes the converse. By
...
deep reverse: every FO $\ZF$-model satisfies Closure & machine-checked & \texttt{DeepReverse.v} \\
$\T \proves \varphi \Rightarrow \ZF \proves \varphi$ & machine-checked & \texttt{DeepReverse.v} \\
$\T \proves \varphi \Leftrightarrow \ZF \proves \varphi$ & machine-checked & \texttt{DeepReverse.v} \\
\bottomrule
\end{tabular}
\end{center}
...
There is no \cq{Admitted} and no custom \cq{Axiom} anywhere in the five files.
...
The development is five Rocq/Coq files under \texttt{src/SetTheory/}.
...
\texttt{DeepReverse.v} & the deep reverse: internal set algebra with \cq{kpair_inj}; internal $\omega$ and the definable-induction schema \cq{omega_ind}; the formula-macro library; \cq{Approx}/\cq{fApproxF} with existence and agreement; \cq{theta_functional}; \cq{ClosureFO_of_ZF}; \cq{ZFmodel_sat_T}; \cq{T_implies_ZF}; \cq{T_iff_ZF}. \\
\bottomrule
\end{tabular}
\end{center}
...
\texttt{DeepReverse.v} \cq{Require}s both.
