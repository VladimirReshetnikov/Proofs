...
four derivations as one schema-instance family, and isolate the one axiom that
genuinely powers the trade --- Powerset --- while showing that Regularity, like
Choice, is a passenger of the equivalence, used in neither direction. The second is
...
machine certify this load-bearing role automatically, and --- on the other side ---
certify that Powerset is \emph{idle} in the reverse direction. Powerset turns out to
be the \emph{only} axiom that genuinely powers the equivalence: as
\S\ref{sec:reverse} shows, the reverse direction needs neither Powerset nor
Regularity, so Regularity (like Choice) merely rides along.
...
genuinely \emph{asymmetric} --- load-bearing forward, idle in reverse --- is
Powerset.
\end{remark}
...
Remark~\ref{rem:mirror}, and with it the asymmetry: Powerset is load-bearing
forward and idle in reverse, while Regularity is idle in \emph{both} directions ---
a passenger of the trade alongside Choice.
...
transitive closures into general collection. And the proof assistant pins down
exactly how much is doing the work, by reading off which hypotheses each derivation
discharged: \emph{Powerset is the single axiom that genuinely powers the trade} ---
load-bearing forward, idle in reverse --- while Regularity, like Choice, turns out to
be a passenger used in neither direction (the reverse needs only the Foundation-free
distinctness of the finite numerals). What looked at first like a Powerset/Regularity
mirror was an artifact of a convenient proof; the machine-checked
$\ZF - \text{Powerset} - \text{Regularity} \proves \text{Closure}$ sets it straight.
