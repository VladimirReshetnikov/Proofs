...
set. Keeping Extensionality, Regularity, Separation, Powerset, and Choice, we show
that $\T$ is \emph{exactly} $\ZF(\mathsf C)$. The forward half (the four deleted
axioms are theorems of $\T$) is the interesting direction; it rests on one small
fact --- Powerset gives every set a host --- which turns Closure into a fully
general principle of collection. The reverse half (every $\ZF$ model satisfies
...
Regularity, Separation, Powerset, Choice --- and add the following schema.
...
\begin{quote}\itshape
$\T$ and $\ZF$ have exactly the same theorems. With the shared axiom of Choice,
$\T$ and $\ZFC$ coincide.
\end{quote}
...
which makes them go is not Closure by itself but Closure standing on
\textbf{Powerset}. The single fact
\[
  a \subseteq a \quad\Longrightarrow\quad a \in \Pow(a)
\]
--- ``Powerset gives every set a host'' --- is what lets us certify that the
relations we care about are set-like, and hence what upgrades Closure from a
...
four derivations as one schema-instance family, and isolate the role of Powerset
(and, by a pleasant mirror symmetry, the role of Regularity in the reverse
direction). The second is to explain, in tutorial style, the
...
theories, the forward direction (with the Powerset remark), the reverse direction,
...
\item[Choice] (in the $\mathsf C$-variants) every set of nonempty sets has a choice function.
\end{description}
...
\begin{remark}[Choice is a passenger]
Choice is stated identically in both theories and is never used in the trade
between the generative axioms and Closure. The equivalence therefore reduces to the
purely ``generative'' content: over the common base
$\{\text{Ext},\text{Sep},\text{Pow},\text{Reg}\}$, the four axioms
$\{\text{Pairing},\text{Union},\text{Infinity},\text{Replacement}\}$ are
interderivable with the single schema Closure.
\end{remark}
...
derive Pairing, Union, Replacement, and Infinity. (Regularity and Choice are not
needed here; they are genuine passengers, as the dependency audit in
\S\ref{sec:mech-shallow} will certify.)

\subsection{The linchpin: Powerset hosts every set}
...
Lemma~\ref{lem:self} looks too small to matter, but it is the entire engine.
Consider any class relation $\prec$ that is \emph{singleton-valued} above each node
--- i.e.\ for each $x$ there is at most one $z$ with $z \prec x$, given by some
class function $z = G(x)$. Is such a $\prec$ set-like? The set-like condition
\eqref{eq:setlike} asks for a set $y$ bounding $\{\,z : z \prec x\,\} = \{G(x)\}$.
By Lemma~\ref{lem:self}, $G(x) \in \Pow(G(x))$, so $y = \Pow(G(x))$ works
\emph{for free}. More generally, whenever the predecessor-class above each node is
already known to sit inside a definable set, Powerset hands us the bound.

\begin{quote}\itshape
This is the conceptual heart of the whole equivalence: \textbf{Powerset turns
``has a definable predecessor'' into ``is set-like.''} Closure then collects the
predecessors. The four generative axioms are four ways of choosing the relation
$\prec$ and the seed $s$.
\end{quote}
...
$x \notin a$), bounded by $\Pow(F(x))$ via Lemma~\ref{lem:self} --- this is where
Powerset is used. Closure applied to $a$ yields a $w$ containing the whole image
$\{F(x) : x \in a\}$, and Separation carves out exactly that image.

\paragraph{Pairing} is the subtle one, and it is precisely where Powerset's
host-providing role becomes visible as a \emph{design constraint}. We want
...
seed $\{\emptyset,\{\emptyset\}\}$ (built from $\emptyset$ and $\Pow$ alone) and
the relation
\[
...
$\emptyset$, $\{b\}$ above $\{\emptyset\}$ --- so Lemma~\ref{lem:self} makes it
set-like ($\Pow(a)$ and $\Pow(b)$ are the hosts), with no circular appeal to
Pairing. Closure of the seed then drags both $a$ and $b$ into one set $w$, and
...
predecessor-class is again a singleton, hostable by Powerset; the relation is
set-like. Closure applied to $\{\emptyset\}$ gives a set $w$ containing
...
\subsection{Powerset is load-bearing}\label{sec:powerset-loadbearing}

The four derivations above use Powerset in a single, uniform role: as the source
of hosts in Lemma~\ref{lem:self}, the fact that makes singleton-valued (and more
generally suitably bounded) relations set-like. Union is the lone exception --- its
relation $z \in x$ is bounded by $x$ directly, with no powerset needed --- which is
why the audit in \S\ref{sec:mech-shallow} will show Union depending on Separation
and Closure \emph{only}.

This is worth stating sharply, because it is easy to read Closure as ``the''
replacement for the four axioms and miss that it is \emph{Closure together with
Powerset} that does the work. Strip Powerset away and the set-like hypothesis of
Closure stops being free: there is no longer a uniform host for a
singleton-valued relation, and the collection power evaporates. We will see the
machine certify this load-bearing role automatically, and --- by a pleasing
symmetry --- certify that Powerset is \emph{idle} in the reverse direction, where
Regularity becomes the load-bearing axiom instead.
...
\]
The bracketed predecessor-set is genuinely a set: set-likeness gives, for each
$v$, a bounding set $b(v)$ of $\{u : u \mathrel R v\}$; Replacement collects
$\{b(v) : v \in t\}$, Union flattens it, and Separation carves out the actual
predecessors. So $g : V \to V$ is a definable total operation.
...
whenever $m < n$, together with the fact that \emph{no set is a member of itself}
--- which is precisely where \textbf{Regularity} enters (apply Regularity to
$\{x\}$ to refute $x \in x$). Finally, $w$ contains $s = W_0$ and is closed under
$R$-predecessors: if $u \mathrel R v$ and $v \in w$, then $v \in W_n$ for some
$n$, hence $u \in g(W_n) = W_{n+1} \subseteq w$.

\begin{remark}[The reverse mirror: Regularity load-bearing, Powerset idle]
\label{rem:mirror}
Notice what powered the reverse construction: Replacement, Union, Infinity (to
collect the family) and Regularity (to make the numerals injective). Powerset was
\emph{not} used. The machine audit in \S\ref{sec:mech-shallow} confirms this,
yielding the sharper statement
\[
  \ZF - \text{Powerset} \ \proves\ \text{Closure}.
\]
So the two directions exhibit a clean mirror image:
\textbf{Powerset is load-bearing forward and idle in reverse; Regularity is idle
forward and load-bearing in reverse.}
\end{remark}
...
Over the common base $\{\text{Extensionality}, \text{Separation},
\text{Powerset}, \text{Regularity}\}$ (and the shared Choice), the theory $\T$
(adding Closure) and the theory $\ZF$ (adding Pairing, Union, Infinity,
Replacement) have exactly the same theorems. In particular $\T$ and $\ZFC$ are the
same theory.
\end{theorem}
\begin{proof}
...
Lemma~\ref{lem:self} is a two-line Coq proof, and it is exactly the pivot the
informal argument advertised:
...
supplies $\Pow(a)$ or $\Pow(b)$ as the host accordingly --- the Coq transcription
of the ``split $a$ and $b$ across different nodes'' argument from
\S\ref{sec:forward}.

\subsection{The free dependency audit}

Here is the part that turns the ``Powerset is load-bearing'' remark from rhetoric
into a machine certificate. When a Coq \cq{Section} closes, each theorem is
generalized over precisely the hypotheses it referenced; the trailing \cq{Check}
commands print those generalized types, so we can simply \emph{read off} which
axioms each derivation used:
...
\item \textbf{Union} depends on \emph{Separation and Closure only} --- not
  Powerset, not Extensionality, not the nonempty-domain assumption. (Its relation
  $z\in x$ is self-bounding.)
\item \textbf{Replacement} depends on \emph{Separation, Powerset, and Closure} ---
  Powerset in exactly its host-providing role.
\item \textbf{Pairing} and \textbf{Infinity} additionally need
  \emph{Extensionality} and a \emph{nonempty domain} (to build $\emptyset$ and to
  tell $\emptyset \neq \{\emptyset\}$ apart).
\item \textbf{Regularity} is referenced by \emph{none} of the four --- a genuine
  passenger, just as \S\ref{sec:forward} claimed.
\end{itemize}
...
Pairing, Union, Infinity, Replacement, and \textbf{Regularity} --- but
\textbf{not Powerset}. This is the machine confirming the sharper
$\ZF - \text{Powerset} \proves \text{Closure}$ and the forward/reverse mirror of
Remark~\ref{rem:mirror}: Powerset load-bearing forward and idle in reverse,
Regularity idle forward and load-bearing in reverse.
...
The non-self-membership fact --- the load-bearing use of Regularity --- and the
injectivity of the numerals it powers are:
...
\cq{onat_inj} is what makes the map ``numeral $\mapsto$ iterate'' well defined,
so Replacement can collect $\{W_n\}$ into a set and Union flatten it. The headline
theorem is then
...
\subsection{Where classical logic enters}
...
a bounding function from set-likeness and to index the iteration. Set theory is
classical anyway and Choice rides along, so this is exactly the intended
metatheoretic setting.
...
that make the quotient an honest type. In particular, the metatheory is \emph{not}
assumed to be stronger than $\ZF$ --- the development does not, for instance, rely
on a universe of $\ZF$-sets supplied by a library.
...
Closure alone but from Closure resting on Powerset, through the one-line fact that
every set is a member of its own powerset. That fact makes ``has a definable
predecessor'' imply ``is set-like'' for free, which is what turns a statement about
transitive closures into general collection. The forward/reverse symmetry is a
small gem: Powerset is the load-bearing axiom forward and idle in reverse, while
Regularity is idle forward and load-bearing in reverse --- and the proof assistant
\emph{certifies} this for us, by reading off exactly which hypotheses each
...
File & Contents \\
\midrule
\texttt{Forward.v} & shallow embedding; $\{$Ext, Sep, Pow, Closure$\}$ derive Pairing/Union/Replacement/Infinity; the linchpin \cq{self_in_power}; dependency-audit \cq{Check}s. \\
\texttt{Reverse.v} & shallow embedding; $\ZF$ derives Closure via the transitive-closure recursion; \cq{no_self_mem}, \cq{onat_inj}; audit shows Powerset unused. \\
