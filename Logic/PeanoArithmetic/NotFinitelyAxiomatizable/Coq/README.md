...
The compactness layer also supports the independently audited theorem
`PATwoNonisomorphicModels.peano_arithmetic_has_two_nonisomorphic_models`:
the standard model is numeral-generated, while the compactness model contains
an element above every standard numeral.
...
- `TwoNonisomorphicModels.v` packages the standard and compactness structures
  as genuine first-order PA models and proves them non-isomorphic using the
  isomorphism-invariant property of being generated by standard numerals.
...
FiniteBetaCoding.v
NonstandardHFFin.v
TwoNonisomorphicModels.v
TwoNonisomorphicModelsAudit.v
...
coqc @q Logic/PeanoArithmetic/NotFinitelyAxiomatizable/Coq/TwoNonisomorphicModelsAudit.v
```
...
coqchk -silent @q PAFiniteBasisReduction.TwoNonisomorphicModels
```
