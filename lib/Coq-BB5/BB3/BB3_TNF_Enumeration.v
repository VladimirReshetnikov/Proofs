...
(** Run the Tree Normal Form Enumeration.
    ...
    Upstream uses [native_compute] here and a later [native_cast_no_check] to
    connect this cached value back to [q_200_def].  This vendored copy uses VM
    conversion for both steps, so the cache equation below is checked by Rocq's
    kernel rather than accepted by an unchecked native cast. *)
Definition q_200 : SearchQueue := Eval vm_compute in q_200_def.
...
Proof.
  assert (q_200 = q_200_def) as H by (vm_compute; reflexivity).
